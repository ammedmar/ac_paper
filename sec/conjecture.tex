...
We reduce $x^{-1}w$ freely and cyclically,\anibal{What does freely means? Doesn't cycliccally here mean what you explain next?} and if two presentations have the same fixed $n$, but different words $w$ and $w'$ such that letters of $x^{-1} w$ can be cyclically permuted to obtain $x^{-1} w'$, we keep only one of these presentations.
